...
public final class OutputStream extends Stream {
	...
	public void skip(int length) {
		this.setOffset(this.getIndex() + length);
	}
	...
	public void writeBytes(byte[] b, int offset, int length) {
		this.checkCapacityPosition(this.getIndex() + length - offset);
		System.arraycopy(b, offset, this.getBuffer(), this.getIndex(), length);
		this.setOffset(this.getIndex() + (length - offset));
	}
	...
	public void writeString(String s) {
		this.checkCapacityPosition(this.getIndex() + s.length() + 1);
		System.arraycopy(s.getBytes(), 0, this.getBuffer(), this.getIndex(), s.length());
		this.setOffset(this.getIndex() + s.length());
		this.writeByte(0);
	}
	...
	public void writePacketVarByte(int id) {
		this.writePacket(id);
		this.writeByte(0);
		this.opcodeStart = this.getIndex() - 1;
	}

	public void writePacketVarShort(int id) {
		this.writePacket(id);
		this.writeShort(0);
		this.opcodeStart = this.getIndex() - 2;
	}

	public void endPacketVarByte() {
		this.writeByte(this.getIndex() - (this.opcodeStart + 2) + 1, this.opcodeStart);
	}

	public void endPacketVarShort() {
		int size = this.getIndex() - (this.opcodeStart + 2);
		this.writeByte(size >> 8, this.opcodeStart++);
		this.writeByte(size, this.opcodeStart);
	}

	public void initBitAccess() {
		this.bitPosition = this.getIndex() * 8;
	}
