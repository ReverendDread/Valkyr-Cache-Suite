...
public final class InputStream extends Stream {
	...
	public void initBitAccess() {
		this.bitPosition = this.index * 8;
	}

	public void finishBitAccess() {
		this.index = (7 + this.bitPosition) / 8;
	}
	...
	public void checkCapacity(int length) {
		if (this.index + length >= this.buffer.length) {
			byte[] newBuffer = new byte[(this.index + length) * 2];
	...
	public int getRemaining() {
		return this.index < this.length ? this.length - this.index : 0;
	}

	public void addBytes(byte[] b, int offset, int length) {
		this.checkCapacity(length - offset);
		System.arraycopy(b, offset, this.buffer, this.index, length);
	...
	public int readByte() {
		return this.getRemaining() > 0 ? this.buffer[this.index++] : 0;
	}
	...
	public int readInt() {
		return (this.readUnsignedByte() << 24) + (this.readUnsignedByte() << 16) + (this.readUnsignedByte() << 8)
	...
	public int readIntV1() {
		return (this.readUnsignedByte() << 8) + this.readUnsignedByte() + (this.readUnsignedByte() << 24)
				+ (this.readUnsignedByte() << 16);
	}

	public int readIntV2() {
		return (this.readUnsignedByte() << 16) + (this.readUnsignedByte() << 24) + this.readUnsignedByte()
				+ (this.readUnsignedByte() << 8);
	}

	public int readIntLE() {
		return this.readUnsignedByte() + (this.readUnsignedByte() << 8) + (this.readUnsignedByte() << 16)
	...
	public int readShortSmart() {
		int value = this.buffer[this.index] & 255;
	...
	public int readUnsignedSmart() {
		int i = 255 & this.buffer[this.index];
		return i >= 128 ? -32768 + this.readUnsignedShort() : this.readUnsignedByte();
	}

	public int read24BitUnsignedInteger() {
		index += 3;
		return ((buffer[index - 1] & 0xff) + (((buffer[index - 3] & 0xff) << 16) + ((buffer[index - 2] & 0xff) << 8)));
	}
